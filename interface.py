...
#                 print(line, end='')
    ...
    def login(self):
        ...
        if login == None: # Condition if Username was not found
            print(RED + 'The Username and Password combination you have entered is incorrect. Please check and try again.')
            self.login()
            ...
            if self.entered_username != inpt_username: # if the entered username differs from the last tried one, reset failed attempts to 1
                ...
                if self.failed_attempts < 3:
                    ...
                    print(RED + 'Your account has been locked because it reached a maximum amount of failed login attempts.')
                    print(RED + 'Please contact the system administrator team for further assistance.\n')
                    quit()
        ...
        if result != None:
            if result[5] == 3:
                ...
                if choice == 'y':
                    ...
                    if unlocked:
                        print(GREEN + "User has successfully been unlocked! The User can now login again.")
                    else:
    ...
    def logout(self):
        ...
        print(BLUE + f"\nThank you for using the NCSC Suspect Sources System. See you soon, {self.first_name}!")
        quit()
    ...
    def search_string_input(self, messege) -> str:
        ...
        if len(input_text) < MIN_LEN: # checks the length of the entered text
            print(RED + "Error: Search term shuld be more than or equal to three charactors.")
